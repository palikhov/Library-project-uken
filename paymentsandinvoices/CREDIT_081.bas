...
Public Sub Invoice_clearing3()


On Error GoTo InvClr_Processing_Err
'==============================================================================================================

ThisWorkbook.Sheets("invoices").Activate
Sheets("invoices").Cells.Select
Range("A1").Activate
Selection.QueryTable.Delete
Selection.Clear
'==============================================================================================================
InvClr_Processing_Err:

     Resume Next
     
'==============================================================================================================
End Sub
Public Sub Control_Processing()
'==============================================================================================================
'==============================================================================================================
' Module    : Control_Processing
' Author    : Антон Палихов aka Palant | http://palikhov.wordpress.com | e-mail: a.v.palikhov@hotmail.com
' Client    : NPC Ukrenergo
'==============================================================================================================
'==============================================================================================================
'Определение переменных
Dim i, j, k, m As Integer ' Счктчики для циклов

Dim cRegistryRowsCount, cStatementRowsCount As Double 'Количество строк на листах реестра и выписки

Dim cRegistryControlRowsCounter As Double 'Счетчик строк, которые необходимо обработать

Dim cStatementControlRowsCounter As Double 'Счетчик строк выписки, которые обработаны

Dim Control_counter As Integer 'Счетчик заполненных строк на листе обработки контроля


'Присвоение начальных значений

cRegistryControlRowsCounter = 0
cStatementControlRowsCounter = 0
Control_counter = 0
'Определение размеров массивов на листах по последней строке
...
'Описание алгоритма: проходим по каждой строке на листе реестра и проверяем наличие обработанных платежей по конкретной строке по наличию общей суммы. Если общая сумма не равна 0, то проверяем значение контрольного значения - если оно равно 0, то переходим на лист выписки и проходим по всем строкам выписки проверяя - совпадает ли переменная соответствия на листе выписки с текущей строкой на листе реестра -  в случае успеха, копируем эту строку с листа выписки на лист обработки контрольных значений.

For i = 2 To cRegistryRowsCount

If ThisWorkbook.Sheets("registry").Cells(i, 33).Value = "" Then GoTo Label1 'Обработка исключения - если сумма пустая, то строку пропускаем

If ThisWorkbook.Sheets("registry").Cells(i, 33).Value = 0 Then GoTo Label1 'Обработка исключения - если сумма равна 0, то строку пропускаем

If ThisWorkbook.Sheets("registry").Cells(i, 40).Value = 0 Then GoTo Label1 'Обработка исключения - если контроль равен 0, то строку пропускаем


For j = 2 To cStatementRowsCount

If ThisWorkbook.Sheets("statement").Cells(j, 32) = i Then _

For k = 2 To 22

ThisWorkbook.Sheets("control_processing").Cells(Control_counter + 2, k).Value = ThisWorkbook.Sheets("statement").Cells(j, k).Value

Next k

ThisWorkbook.Sheets("control_processing").Cells(Control_counter + 2, 23).Value = i

ThisWorkbook.Sheets("statement").Cells(j, 33).Value = "Необхідний ручний контроль. Перевірте аркуш control_processing"
...
MsgBox "Всього рядків для перевірки по різниці контрольного значення " & Control_counter
...
Dim i As Double
Dim k As Long
Dim tRowsCount3 As Double

tRowsCount = ThisWorkbook.Sheets("REGISTRY").Cells(Rows.Count, 1).End(xlUp).Row
tRowsCount2 = ThisWorkbook.Sheets("REGISTRY").Cells(Rows.Count, 4).End(xlUp).Row
tRowsCount3 = ThisWorkbook.Sheets("REGISTRY").Cells(Rows.Count, 32).End(xlUp).Row
...
.Cells(i, 1).FormulaR1C1Local = "=VLOOKUP(RC[3];ДОГОВОРИ!R4C2:R1600C4;2;FALSE)"

.Cells(i, 2).FormulaR1C1Local = "=VLOOKUP(RC[2];ДОГОВОРИ!R4C2:R1600C4;3;FALSE)"
.Cells(i, 3).FormulaR1C1Local = "=VLOOKUP(RC[1];ДОГОВОРИ!R4C2:R2000C12;11;FALSE)"
'.Cells(i, 17).FormulaR1C1Local = "=ROUND(RC[-1]*1,2;2)"
.Cells(i, 32).FormulaR1C1Local = "=RC[-30]&RC[-14]"
k = k + 1
Next i



MsgBox "Опрацьовано " & k & " рядків в які додано формули пошуки реквізітів договору по ікс ідентифікатору"
...
' При открытии файла делает бэкап и разрывает все связи
'==============================================================================================================
'==============================================================================================================
' Module    : Statement_Processing
' Author    : Антон Палихов aka Palant | http://palikhov.wordpress.com | e-mail: a.v.palikhov@hotmail.com
...
'********************************************************************************************
    ...
    Dim Outcome_counter As Integer 'Количество строк, в которых отправитель платежа Укрэнерго
...
'Начальные значения переменных
    ...
    Set oReg = regws.Range(Cells(31, 2).Address, Cells(cRowsCount + 1, 79).Address)
'********************************************************************************************
    ...
    Set oReg = regws.Range(Cells(2, 33).Address, Cells(cRegistryRowsCount, 79).Address)
 ...
 '********************************************************************************************
    ...
    For i = 1 To Statement_Rows
            ...
            sum_statement = WorksheetFunction.Substitute(ThisWorkbook.Sheets("statement").Cells(i + 1, 7).Value, " UAH", "")    'Определение суммы платежа на листе выписки
            
            date_payment_i = ThisWorkbook.Sheets("statement").Cells(i + 1, 3).Value                                             'Определение даты платежа на листе выписки

            customer_i = ThisWorkbook.Sheets("statement").Cells(i + 1, 11).Value                                                'Определение контрагента на листе выписки
...
OutcomeProcessing:             'Определение того что платеж является исходящим и должен быть обработан при обработке реестра дебита
            ...
            '********************************************************************************************
                If ThisWorkbook.Sheets("statement").Cells(i + 1, 11).Value = "00100227" Or ThisWorkbook.Sheets("statement").Cells(i + 1, 11).Value = "100227" Then ThisWorkbook.Sheets("statement").Cells(i + 1, 28).Value = "Outcome" Else GoTo AutoProcessing 'Если ЕГРПОУ отправителя равен ЕГРПОУ Укрэнерго - значит это исходящий платеж
            
                Outcome_counter = Outcome_counter + 1 'Увеличение счетчика исходящих платежей
...
AutoProcessing:
            ...
            '============================================================================================
                ...
                If colInvoiceMatches.Count = 0 Then GoTo Income_Semi_Auto_Processing
...
DATE_DETECTION:
        ...
        'ПРОВЕРКА - ЕСЛИ НАйденный инвойс начинается с 0, то и на листе реестра он стоит с 0, а значит форматирование экселя уже съело этот 0 поэтому первый символ необходимо отбросить и перезаписать значение инвойса
            ...
            ThisWorkbook.Sheets("statement").Cells(i + 1, 52).Value = Application.IsError(Application.Match(invoice_i, ThisWorkbook.Sheets("REGISTRY").Range("I1:I100000").Value, 0)) 'ВЫВОДИМ В СТОЛБЕЦ 52 РЕЗУЛЬТАТ ПРОВЕРКИ ПОИСКА ИНВОЙСА НА ЛИСТЕ РЕЕСТРА. В СЛУЧАЕ УСПЕХА В 52 СТОЛБЕЦ ДОЛЖНО ЗАПИСАТЬСЯ FALSE
            ...
            If matchi > 0 Then GoTo Auto Else GoTo Income_Semi_Auto_Processing 'ЕСЛИ ПЕРЕМЕННАЯ БОЛЬШЕ 0 ЗНАЧИТ ЕСТЬ СОПОСТАВЛЕНИЕ - ПЕРЕХОДИМ НА ЗАКЛАДКУ АВТОМАТИЧЕСКОГО СОПОСТАВЛЕНИЯ. В ПРОТИВНОМ СЛУЧАЕ ПЕРЕХОДИМ НА ЗАКЛАДКУ ПОЛУАВТОМАТИЧЕСКОГО СОПОСТАВЛЕНИЯ ЧТОБЫ ПОПЫТАТЬСЯ СОПОСТАВИТЬ ПО НЕСТРОГОМУ ПРИЗНАКУ
...
Income_Semi_Auto_Processing:
            ...
            ThisWorkbook.Sheets("statement").Cells(i + 1, 43).Value = Application.IsError(Application.Match(semi_invoice_i, ThisWorkbook.Sheets("REGISTRY").Range("AE1:AE100000").Value, 0))
            ...
            If Application.IsError(Application.Match(semi_invoice_i, ThisWorkbook.Sheets("REGISTRY").Range("Af1:Af100000").Value, 0)) = True Then semi_invoice_match_i2 = 0 Else semi_invoice_match_i2 = Application.Match(semi_invoice_i, ThisWorkbook.Sheets("REGISTRY").Range("AE1:AE100000").Value, 0)
...
Label1:     Next i
    ...
    MsgBox "Вихідних платежів " & Outcome_counter & " рядків. В автоматичному режимі опрацьовано " & Automated_counter & " рядків. Полуавтоматичне співставлення " & Semi_automated_counter & " рядків. Для ручної обробки залишилось " & Manual_counter & " рядків"
...
If ThisWorkbook.Sheets("invoices").Cells(i + 1, 11) = "CREDIT" Then _
...
oReg.ClearContents
'==============================================================================================================
MsgBox "Проміжні дані на аркуші реєстру очищені"
End Sub


Sub StatusBarChangeOn()

Application.StatusBar = "Миру мир, а Тотоше чай с кексом"

End Sub

Sub StatusBarChangeOff()
...
Application.StatusBar = Space(intSpaces) & "Пусть бегут неуклюже пробелы по статус-бару"
...
Sub WindowTitleCustomOn()

Application.Caption = "Какой-то интересный заголовок у этого окна"

End Sub


Sub WindowTitleCustomOn2()

Application.Caption = "Какой-то интересный заголовок у этого окна"
ActiveWindow.Caption = "Черный Властелин"

End Sub

Sub WindowTitleCustomOff()
...
Set oRangeStatement = ThisWorkbook.Sheets("statement").Range(Cells(2, 28).Address, Cells(cRowsStatementCount + 1, 39).Address)

oRangeStatement.ClearContents

cRegistryRowsCount = ThisWorkbook.Sheets("REGISTRY").Cells(Rows.Count, 2).End(xlUp).Row

Set oReg = regws.Range(Cells(2, 34).Address, Cells(cRegistryRowsCount, 78).Address)
...
Select Case MsgBox("Ця дія не може бути відмінена. Ви заміните в стовпцях 1, 2, 3, та 32 формули на хначення, але це прискорить роботу програми. Ви бажаєте зберігти книгу перед продовженням?", vbYesNoCancel)
Case Is = vbYes
ThisWorkbook.Save
Case Is = vbCancel
Exit Sub
...
Set objRange1 = ThisWorkbook.Sheets("REGISTRY").Range(Sheets("REGISTRY").Cells(2, 1), Sheets("REGISTRY").Cells(tRowsCount2, 1))

For Each objCell In objRange1
objCell.Formula = "=VLOOKUP(RC[3];ДОГОВОРИ!R4C2:R1600C4;2;FALSE)"
Next objCell

Set objRange2 = ThisWorkbook.Sheets("REGISTRY").Range(Sheets("REGISTRY").Cells(2, 2), Sheets("REGISTRY").Cells(tRowsCount2, 2))

For Each objCell In objRange2
If objCell.Formula Like "*!*" Then objCell.Formula = "=VLOOKUP(RC[3];ДОГОВОРИ!R4C2:R1600C4;2;FALSE)"
Next objCell
Set objRange32 = ThisWorkbook.Sheets("REGISTRY").Range(Sheets("REGISTRY").Cells(2, 32), Sheets("REGISTRY").Cells(tRowsCount2, 32))
For Each objCell In objRange32
If objCell.Formula Like "*!*" Then objCell.Formula = objCell.Value
Next objCell
...
MsgBox "Формули замінено на значення", vbButtonType, "Результат"

End Sub

Public Sub valuetoformula()
    '// add declarations
Dim tRowsCount As Double
Dim tRowsCount2 As Double
Dim i As Double
...
Select Case MsgBox("Ця дія не може бути відмінена. Ви заміните в стовпцях 1, 2, 3, та 32 значення на формулу пошуку по ікс ідентифікатору, але це УПОВІЛЬНИТЬ роботу програми. Ви бажаєте зберігти книгу перед продовженням?", vbYesNoCancel)
Case Is = vbYes
ThisWorkbook.Save
Case Is = vbCancel
Exit Sub
...
For Each objCell In objRange1
objCell.FormulaR1C1Local = "=VLOOKUP(RC[3];ДОГОВОРИ!R4C2:R1600C4;2;FALSE)"

Next objCell
Set objRange2 = ThisWorkbook.Sheets("REGISTRY").Range(Sheets("REGISTRY").Cells(2, 2), Sheets("REGISTRY").Cells(tRowsCount2, 2))
For Each objCell In objRange2
objCell.FormulaR1C1Local = "=VLOOKUP(RC[2];ДОГОВОРИ!R4C2:R1600C4;2;FALSE)"
Next objCell

Set objRange3 = ThisWorkbook.Sheets("REGISTRY").Range(Sheets("REGISTRY").Cells(2, 3), Sheets("REGISTRY").Cells(tRowsCount2, 3))
For Each objCell In objRange3
objCell.FormulaR1C1Local = "=VLOOKUP(RC[1];ДОГОВОРИ!R4C2:R1600C4;2;FALSE)"
Next objCell
Set objRange32 = ThisWorkbook.Sheets("REGISTRY").Range(Sheets("REGISTRY").Cells(2, 32), Sheets("REGISTRY").Cells(tRowsCount2, 32))
For Each objCell In objRange32
objCell.FormulaR1C1Local = "=RC[-30]&RC[-14]"
Next objCell
On Error GoTo catchError
exitSub:
    Exit Sub
...
MsgBox "Значення замінено на формули", vbButtonType, "Результат"
